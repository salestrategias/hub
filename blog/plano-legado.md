# Plano de reescrita — artigos legados (era Gemini)

20 artigos anteriores a 09/07/2026. **Decisão do Marcelo (10/08/2026): reangular cada um, sem apagar nenhuma URL.** Slug sempre preservado (autoridade acumulada).

## Padrão de defeito encontrado no lote

Diagnóstico a partir da amostra (1122): abertura com "Resumo:", Title Case em todos os headings (convenção inglesa, errada em pt-BR), adjetivo bombástico ("Alta Performance", "Máximo ROI", "O Criativo Vencedor"), nenhum exemplo numérico, nenhuma fonte datada, `rank_math_title` no template `%title% - SAL`, focus keyword com 5 termos empilhados (o Rank Math só usa o primeiro), sem FAQ e sem resposta direta na abertura.

## O que cada reescrita entrega

1. Resposta direta nos primeiros 50-70 palavras (GEO), sem "Resumo:".
2. Headings em caixa de frase, sem adjetivo de propaganda.
3. Exemplo numérico fechando a conta em reais onde couber.
4. Estatística só com fonte linkada e ano declarado no texto. Na dúvida, corta o número.
5. FAQ (3-5 perguntas) + schema FAQPage.
6. 4-8 links internos para o cluster novo.
7. Meta do Rank Math preenchida de verdade (title 50-60, description 150-160, 1 focus keyword).
8. Sumário TOC regenerado. Capa no modo gráfico de marca.
9. Sem "PME", sem em-dash, sem promessa exagerada.

## Mapa de reangulação

### Grupo Google Meu Negócio (3 URLs, 3 ângulos)
| ID | Slug | Novo ângulo |
|---|---|---|
| 1156 | otimizar-google-meu-negocio-guia-avancado | Guia geral: configuração completa do perfil (pilar do grupo) |
| 1097 | otimizacao-de-google-meu-negocio-2026-2 | Categoria e atributos: a escolha que decide em quais buscas você aparece |
| 1334 | otimizacao-google-meu-negocio-2026-algoritmo-frescor | Frequência e frescor: que ritmo de publicação e atualização manter |

### Grupo SEO local (4 URLs, 4 ângulos)
| ID | Slug | Novo ângulo |
|---|---|---|
| 1026 | como-fazer-seo-para-negocios-locais | Guia geral de SEO local (nacional; POA tem página própria) |
| 1095 | seo-para-negocios-locais-2026-5 | Como medir retorno de SEO local sem sistema caro |
| 1327 | seo-para-pequenas-empresas-2026 | FEITO. Virou "SEO local com pouco tempo: duas horas por semana". Slug preservado; termo proibido zerado na página renderizada |
| 1332 | seo-para-negocios-locais-2026-dominacao-regional | SEO para quem atende várias regiões, bairros ou unidades |

### Grupo SEO / GEO (3 URLs, 3 ângulos)
| ID | Slug | Novo ângulo |
|---|---|---|
| 1067 | o-que-e-seo-como-funciona-guia | Fundamentos: o que é SEO e como funciona (TOFU) |
| 1079 | geo-seo-para-ias-chatgpt-google-sge | O que é GEO e no que difere do SEO (conceitual) |
| 371 | seo-e-geo-2026-o-guia-completo... | Como combinar SEO e GEO no mesmo plano (estratégico) |

### Sem duplicata (11 URLs)
| ID | Slug | Ajuste |
|---|---|---|
| 1122 | como-anunciar-no-meta-ads-em-2026-guia | Reescrita. Remover "PMEs" |
| 1158 | como-anunciar-no-google-ads-em-2026-guia | Reescrita. Remover "PMEs" |
| 1341 | meta-ads-em-2026-impostos-custos | FEITO. Os percentuais antigos (20%, 15%, 25%) estavam sem fonte e errados. Reescrito com o comunicado oficial da Meta: 12,15% (PIS/Cofins 9,25% + ISS 2,9%), R$ 1.000 pagos = R$ 878,50 de entrega |
| 1165 | como-criar-uma-loja-virtual-ecommerce-2026 | Reescrita |
| 1174 | palavras-chave-de-cauda-longa-o-que-sao | Reescrita |
| 1282 | google-analytics-4-guia-completo | Reescrita. Falta TOC, tem em-dash |
| 1285 | planejamento-de-marketing-digital-2026 | Reescrita |
| 1288 | cac-e-ltv-como-calcular-metricas-negocio | FEITO. Link da Forbes estava 404. Reescrito com o achado de que a regra 3:1 vem do SaaS (David Skok) e não serve ao varejo: faixa de 2,5 a 4 para 1 |
| 1299 | o-que-e-inbound-marketing-como-implementar | Reescrita, traduzida para a realidade de lojista |
| 1014 | sal-em-marketing-sales-accepted-lead | FEITO com pesquisa profunda (workflow de 4 agentes + verificação adversarial). Defesa de marca: a sigla SAL colide com o termo técnico e concorrentes capturavam busca por "SAL marketing". Artigo agora tem fonte primária datada (SiriusDecisions 2006), resolve a divergência das 3 escolas do funil, entrega SLA operacional, e desambigua a marca no topo. 2.743 palavras, 6 fontes externas, FAQPage + DefinedTerm |
| 371 | (ver grupo SEO/GEO) | |

## Status

- [x] Lote 1: 1122, 1158
- [x] Lote 2: 1341, 1282, 1288
- [x] Lote 3: grupo Google Meu Negócio (1156, 1097, 1334)
- [x] Lote 4: grupo SEO local (1026, 1095, 1327, 1332)
- [x] Lote 5: grupo SEO/GEO (1067, 1079, 371)
- [ ] Lote 6: 1165, 1174, 1285, 1299, 1014
