...
### 2026-07-13 (tarde) — Portal v5 "app da marca"
- ✅ **Portal v5 — layout aprovado no protótipo** (`preview/portal-v5.html`): a cor do cliente vira protagonista (herói, FAB, nav); tokens `.portal-v5` no globals (ground tingido, cards sem borda com sombra dupla, raio 22px). **Início-feed**: herói fintech de pendências (número grande + thumbs + "Revisar agora"), strip "Sua semana" (bolinhas por dia; âmbar = pendência), carrossel "Para aprovar" (itens leves do /resumo, sem base64), últimas entregas; contadores de mês saíram (foram pra Resultados). **Conteúdo**: vista FEED = grade 3×3 estilo perfil do Instagram agrupada por mês (capa = 1ª imagem ou gradiente; dot de status âmbar/céu/verde; tocar abre o card completo num sheet) ↔ LISTA; grade mensal de calendário aposentada. **Revisão stories**: sempre escura, progresso segmentado, arte grande, chips, ajuste inline, fim com check animado. **Aba Resultados** (nova): 4 métricas do mês com delta MoM (MetricaRede somada entre redes), barras de alcance dos últimos 4 meses, PDFs mensais — endpoint novo `/api/p/cliente/[token]/resultados`. **Nav**: Início · Conteúdo · FAB(+ Enviar em sheet, 2 modos preservados) · Resultados · Mais (sem Relatórios — migrou); deep-link `#enviar`. `/resumo` agora devolve `pendencias.itens` + `semana` + é buscado UMA vez (shell distribui por props — mata o double-fetch da auditoria). `portal-relatorios.tsx` removido (absorvido por Resultados).

### 2026-07-13
- ✅ **Fase 0 — consertos críticos do portal** (da auditoria 2026-07): rota `/api/p/cliente/[token]/relatorio-mensal` autenticada pela sessão do cliente (mata o 401 da aba Relatórios); sessão expirada agora responde **401 real** (`ApiError` em `lib/api.ts` + `requerSessaoCliente`) e o front volta pra tela de senha via evento global (`lib/portal-fetch.ts`) em vez de "portal vazio"; **renovação deslizante** do cookie (7d a cada visita); **rate-limit** na senha do portal (5/15min por token+IP, `lib/rate-limit.ts`); badge de pendência alinhado à janela -30/+60d do calendário; enum `PORTAL_APROVOU_CRIATIVO` (criativo não se passa mais por post); `EMOJI`/`CTA_LABEL` de email completos; índices de FK (`Tarefa.clienteId/projetoId`, `Lancamento.clienteId`, `CheckItem.tarefaId`, `ReuniaoAction/Capitulo.reuniaoId`, `SeoKeyword.clienteId`, `DriveArquivo.clienteId`); `registrarAcesso` só incrementa 1x/30min.
- ✅ **Portal v4 — nova experiência do cliente**: navegação **4 abas** (Início · Conteúdo · Enviar · Mais) no lugar de 7; aba **Conteúdo** unifica posts+criativos (chips Tudo/Orgânico/Anúncios, lista com "Esperando você" primeiro + grade mensal); **Modo Revisão** (`portal-revisao.tsx`) — fila fullscreen item a item com Aprovar/Pedir ajuste/Decidir depois, progresso e tela de conclusão, deep-link `#revisar`; **estados sem ambiguidade** — depois de pedir ajuste o card vira "Com a SAL" (heurística `updatedAt` vs último comentário) e os botões primários somem; **thread completa** de comentários (take 30, estilo conversa); aba **Enviar** com DOIS modos — "Só o material" (dropzone hero: drag&drop + colar + câmera `capture` + galeria, recado opcional, defaults no servidor, SAL produz) e "**Conteúdo pronto**" (modelo consultoria: cliente publica post/anúncio completo — título, legenda, formato, data, hashtags / plataforma, headline — e a SAL só aprova na triagem), "Meus envios" com status; EXIF corrigido via `createImageBitmap({imageOrientation})`, aviso específico pra HEIC; aba **Mais** (Briefings/Relatórios/Reuniões/Tarefas + WhatsApp da SAL); dialogs/carrossel/status unificados em `portal-conteudo-shared.tsx` (morre a duplicação portal-calendario × portal-criativos, arquivos antigos removidos); `window.confirm` substituído por dialog próprio; erros de rede ganham "Tentar de novo"; jargão traduzido ("Copy/Legenda"→"Legenda", "Primary Text"→"Texto do anúncio", pilar não vaza). Permissões do portal agora respondem **403** (`ApiError`).
- ⏭️ Próximas fases da auditoria: F1 notificações por e-mail nas duas direções + cron (depende do Resend, ver "Em espera"), F2 storage de arquivos no VPS (destrava vídeo grande), F4 espinha comercial (aceite→contrato), F5 consolidações internas.

### 2026-05-13
- ✅ **Posts editoriais enriquecidos** — campos novos no `Post`: `hashtags[]`, `cta`, `observacoesProducao`. Novo model `PostArquivo` (artes/anexos com tipos IMAGEM/VIDEO/DOCUMENTO/LINK_EXTERNO, ordem manual). Editor admin no post-sheet com 5 tabs (Copy + Artes + Hashtags+CTA + Notas de produção + Comentários do cliente). Upload com compressão de imagem (canvas, max 1600px JPEG 80%). Drag-drop pra reordenar arquivos. Portal do cliente mostra carrossel de artes (swipe touch + setas desktop + dots), copy rica (BlockRenderer), hashtags como chips, CTA destacado.
