"use client";
/**
 * Editor de artes/anexos do criativo (usado no criativo-sheet).
 *
 * Mesma mecânica do PostArquivosEditor (upload com compressão + URL
 * externa + drag-drop reorder), mas escopo é Criativo. Mantido separado
 * pra não acoplar evolução do tráfego pago com a do editorial.
 */
import { useEffect, useRef, useState } from "react";
import { DragDropContext, Droppable, Draggable, type DropResult } from "@hello-pangea/dnd";
import { FileText, Link2, Trash2, GripVertical, Upload, Loader2, Plus, Video } from "lucide-react";
...
export function CriativoArquivosEditor({ criativoId }: { criativoId: string }) {
  ...
  async function carregar() {
    setCarregando(true);
    try {
      const res = await fetch(`/api/criativos/${criativoId}/arquivos`);
  ...
  async function uploadArquivo(file: File) {
    ...
    try {
      ...
      const res = await fetch(`/api/criativos/${criativoId}/arquivos`, {
        ...
        body: JSON.stringify({ tipo, url, nome: file.name }),
      });
      if (!res.ok) {
  ...
  async function excluir(id: string) {
    ...
    const res = await fetch(`/api/criativos/arquivos/${id}`, { method: "DELETE" });
    if (!res.ok) {
  ...
  async function atualizarLegenda(id: string, legenda: string) {
    await fetch(`/api/criativos/arquivos/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function onDragEnd(r: DropResult) {
    ...
    try {
      await fetch(`/api/criativos/${criativoId}/arquivos/reordenar`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0 space-y-1.5">
                          ...
                          <Input
                            ...
                            placeholder="Legenda (ex: 'Variante A — fundo escuro')"
