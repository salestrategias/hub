...
import {
  MousePointer2, Hand, Square, Circle, Type, Minus, MoveRight, StickyNote,
  Plus, Minus as MinusIcon, Maximize, Copy, Trash2, Save, Download,
} from "lucide-react";
...
export function MindMapCanvas({
  id, titulo: tituloInicial, descricao,
  data: dataInicial,
}: {
  id: string;
  titulo: string;
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(async () => {
      const res = await fetch(`/api/mapas/${id}`, {
        ...
        body: JSON.stringify({ titulo, data: { nodes, edges } }),
      });
  ...
  const handleCanvasClick = useCallback(
    (e: React.MouseEvent<SVGSVGElement>) => {
      ...
      const novo: Node = {
        ...
        texto: tool === "sticky" ? "Sticky note" : tool === "text" ? "Texto" : "Novo nó",
  ...
  function startDrag(e: React.MouseEvent, n: Node) {
    ...
    const pt = svgPoint(e);
    if (!pt) return;
    dragState.current = { nodeId: n.id, offsetX: pt.x - n.x, offsetY: pt.y - n.y };
    setSelectedId(n.id);
  }
        ...
        setNodes((prev) =>
          prev.map((n) =>
            n.id === dragState.current!.nodeId
              ? { ...n, x: pt.x - dragState.current!.offsetX, y: pt.y - dragState.current!.offsetY }
              : n
          )
        );
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          <input
            ...
            className="bg-transparent font-display font-semibold text-lg outline-none"
            style={{ width: `${Math.max(titulo.length, 12)}ch` }}
          />
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={exportarSvg}><Download className="h-3.5 w-3.5" /> Exportar SVG</Button>
        </div>
      </div>
      ...
      <Card className="overflow-hidden relative p-0" style={{ height: "calc(100vh - 240px)" }}>
        ...
        <div className="absolute left-3 top-3 z-10 bg-card border border-border rounded-lg p-1.5 flex flex-col gap-0.5 shadow-lg">
          {([
            ["select", MousePointer2, "Selecionar"],
            ["pan", Hand, "Mover canvas"],
            ["rect", Square, "Retângulo"],
            ["circle", Circle, "Círculo"],
            ["text", Type, "Texto"],
            ["arrow", MoveRight, "Conectar"],
        ...
        {selected && (
          <div className="absolute right-3 top-3 z-10 bg-card border border-border rounded-lg p-3 w-56 shadow-lg space-y-3">
              ...
              <div className="flex gap-1 flex-wrap">
                {CORES.map((c) => (
                  <button
                    ...
                    className={cn("w-5 h-5 rounded-full border-2 transition", selected.cor === c ? "border-foreground scale-110" : "border-transparent")}
                    style={{ background: c }}
                  />
                ))}
              </div>
            ...
            <div className="border-t border-border pt-3 flex justify-between gap-1">
              ...
              <Button size="icon" variant="ghost" className="h-8 w-8" onClick={duplicar} title="Duplicar"><Copy className="h-3.5 w-3.5" /></Button>
              <Button size="icon" variant="ghost" className="h-8 w-8" onClick={deletarSelecionado} title="Excluir"><Trash2 className="h-3.5 w-3.5" /></Button>
            </div>
        ...
        <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-10 bg-card border border-border rounded-lg px-2 py-1 flex items-center gap-1 shadow-lg">
          <button className="p-1.5 rounded hover:bg-secondary" onClick={() => setZoom(Math.max(0.3, zoom - 0.1))}><MinusIcon className="h-3.5 w-3.5" /></button>
          <span className="text-[11px] font-mono px-2">{Math.round(zoom * 100)}%</span>
          <button className="p-1.5 rounded hover:bg-secondary" onClick={() => setZoom(Math.min(2, zoom + 0.1))}><Plus className="h-3.5 w-3.5" /></button>
          <div className="w-px h-4 bg-border mx-1" />
          <button className="p-1.5 rounded hover:bg-secondary" onClick={() => { setZoom(1); setPan({ x: 0, y: 0 }); }}><Maximize className="h-3.5 w-3.5" /></button>
        </div>
        ...
        <svg
          ...
          style={{ cursor: tool === "pan" ? "grab" : tool === "select" ? "default" : "crosshair", background: "hsl(var(--card))" }}
        >
          <defs>
          ...
          <g transform={`translate(${pan.x}, ${pan.y}) scale(${zoom})`}>
            ...
            {nodes.map((n) => (
              ...
              >
                {n.tipo === "circle" ? (
                  <ellipse cx={n.w / 2} cy={n.h / 2} rx={n.w / 2} ry={n.h / 2} fill={`${n.cor}22`} stroke={n.cor} strokeWidth={selectedId === n.id ? 3 : 1.5} />
                ) : n.tipo === "sticky" ? (
                  <rect width={n.w} height={n.h} rx="3" fill={n.cor} transform="rotate(-2)" stroke={selectedId === n.id ? "#7E30E1" : "transparent"} strokeWidth={2} />
                ) : (
                  <rect width={n.w} height={n.h} rx="10" fill={n.tipo === "text" ? "transparent" : `${n.cor}22`} stroke={n.cor} strokeWidth={selectedId === n.id ? 2.5 : 1.5} />
                )}
                <text
                  x={n.w / 2}
      ...
      <div className="text-[11px] text-muted-foreground/60 text-center">
        Selecione uma ferramenta e clique no canvas pra criar · Arraste nós com a ferramenta de seleção · Duplo clique edita · <span className="kbd-key">Shift</span> + arrastar move o canvas
      </div>
    </div>
  );
}
