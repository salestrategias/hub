...
const acessoSchema = z.object({
  ...
  verCriativos: z.boolean().optional(),
...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  return apiHandler(async () => {
    ...
    return {
      ...
      verCriativos: acesso.verCriativos,
  ...
  return apiHandler(async () => {
    ...
    if (existente) {
      const upd = await prisma.clienteAcesso.update({
        ...
        data: {
          ...
          ...(body.verCriativos !== undefined ? { verCriativos: body.verCriativos } : {}),
    ...
    const novo = await prisma.clienteAcesso.create({
      data: {
        ...
        verCriativos: body.verCriativos ?? true,
