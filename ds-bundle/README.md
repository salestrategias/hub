...
- `styles.css` — ponto de entrada; importa `tokens/tokens.css` e `_ds_bundle.css`.
- `tokens/tokens.css` — paleta Marinho & Coral (escuro padrão + tema claro), tipografia, espaço, forma e movimento. Fontes em `fonts/`.
- `_ds_bundle.css` — o CSS real do site www.salestrategias.com.br (Astro) mais as peças de marca (`.capa`, `.capa-social`).
- `components/` — cartões de referência com exemplos verificados: Fundação (Cores, Tipografia, Grão de sal), Ações (Botões), Blocos (Card de serviço, Card de blog, Linha de índice, Chip de logo, CTA final) e Marca (Capa de post 1200×675, Card social 1080×1080).

Origem: repositório `salestrategias/hub`, `sal-site/src/styles/`. Sistema CSS puro — não há bundle JS de componentes; construa com as classes e tokens acima.
